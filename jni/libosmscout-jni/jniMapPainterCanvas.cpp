...
namespace osmscout {
  ...
  MapPainterCanvas::MapPainterCanvas() : MapPainter(new CoordBufferImpl<Vertex2D>()),
                                 ...
                                 IconStyle& style)
  {
         ...
         path!=parameter.GetIconPaths().end(); ++path)
    {
      ...
      bool loaded=mJniEnv->CallBooleanMethod(mPainterObject, methodId,
                                            iconName, iconIndex);
...
//    }
                  ...
                  size_t transStart, size_t transEnd)
  {
    ...
    for(int i=0; i<numPoints; i++)
    {
    	x[i]=(float)transBuffer.transPolygon.points[transStart+i].x;
    	y[i]=(float)transBuffer.transPolygon.points[transStart+i].y;
    }
                  ...
                  const AreaData& area)
  {
    ...
    if (!area.clippings.empty())
    {
          ...
          c!=area.clippings.end(); c++)
      {
        ...
        for(int i=0; i<numPoints; i++)
        {
          x[i]=(float)transBuffer.transPolygon.points[clipData.transStart+i].x;
          y[i]=(float)transBuffer.transPolygon.points[clipData.transStart+i].y;
        }
    ...
    for(int i=0; i<numPoints; i++)
    {
    	x[i]=(float)transBuffer.transPolygon.points[area.transStart+i].x;
    	y[i]=(float)transBuffer.transPolygon.points[area.transStart+i].y;
    }
  ...
  void  MapPainterCanvas::DrawGround(const Projection& projection,
                             const MapParameter& parameter,
                             const FillStyle& style){
