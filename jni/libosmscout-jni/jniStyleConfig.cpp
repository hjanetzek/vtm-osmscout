...
extern "C" {
   ...
   jint
   Java_osm_scout_StyleConfig_jniConstructor(JNIEnv *env, jobject object, int typeConfigIndex) {
      TypeConfig *nativeTypeConfig = gTypeConfigArray->Get(typeConfigIndex);

      if (!nativeTypeConfig) {
	 printf("jniConstructor(): NULL TypeConfig object");
   ...
   void
   Java_osm_scout_StyleConfig_jniDestructor(JNIEnv *env, jobject object, int styleConfigIndex) {
   ...
   jboolean
   Java_osm_scout_StyleConfig_jniLoadStyleConfig(JNIEnv *env, jobject object, int styleConfigIndex,
   ...
   jobject
   Java_osm_scout_StyleConfig_jniGetObjectTypesWithMaxMag(JNIEnv *env, jobject object,
	 int styleConfigIndex, jdouble magnification) {
      StyleConfig *nativeStyleConfig = gStyleConfigArray->Get(styleConfigIndex);

      if (!nativeStyleConfig) {
	 printf("jniGetObjectTypesWithMaxMag(): NULL StyleConfig object");
	 return NULL;
      }
      ...
      nativeStyleConfig->GetNodeTypesWithMaxMag(magnification, nativeObjectTypeSets->nodeTypes);

      nativeStyleConfig->GetWayTypesByPrioWithMaxMag(magnification, nativeObjectTypeSets->wayTypes);

      nativeStyleConfig->GetAreaTypesWithMaxMag(magnification, nativeObjectTypeSets->areaTypes);
