...
extern "C" {
   ...
   jint
   Java_osm_scout_Database_jniConstructor(JNIEnv *env, jobject object) {
   ...
   void
   Java_osm_scout_Database_jniDestructor(JNIEnv *env, jobject object, int databaseIndex) {
      Database *nativeDatabase = gDatabaseArray->GetAndRemove(databaseIndex);

      if (!nativeDatabase) {
	 printf("jniDestructor(): NULL Database object");
      } else
	 delete nativeDatabase;
   }

   jboolean
   Java_osm_scout_Database_jniOpen(JNIEnv *env, jobject object, int databaseIndex,
	 jstring javaPath) {
      Database *nativeDatabase = gDatabaseArray->Get(databaseIndex);

      if (!nativeDatabase) {
	 printf("jniOpen(): NULL Database object");
	 return JNI_FALSE;
      }
   ...
   jboolean
   Java_osm_scout_Database_jniIsOpen(JNIEnv *env, jobject object, int databaseIndex) {
      Database *nativeDatabase = gDatabaseArray->Get(databaseIndex);

      if (!nativeDatabase) {
	 printf("jniIsOpen(): NULL Database object");
	 return JNI_FALSE;
      }
   ...
   jobject
   Java_osm_scout_Database_jniGetBoundingBox(JNIEnv *env, jobject object, int databaseIndex) {
      Database *nativeDatabase = gDatabaseArray->Get(databaseIndex);

      if (!nativeDatabase) {
	 printf("jniGetBoundingBox(): NULL Database object");
	 return NULL;
      }
	 ...
	 printf("jniGetBoundingBox(): GetBoundingBox() returned FALSE");
	 return NULL;
      }
   ...
   Java_osm_scout_Database_jniGetMatchingAdminRegions(JNIEnv *env, jobject object,
	 ...
	 jboolean startWith) {
      ...
      if (!nativeDatabase) {
	 printf("jniGetMatchingAdminRegions(): NULL Database object");
	 return NULL;
      }
      ...
      if (nativeDatabase->GetMatchingAdminRegions(nativeName, nativeRegions, limit,
	    nativeLimitReached, startWith)) {
	 ...
	 for (std::list<osmscout::AdminRegion>::iterator iter = nativeRegions.begin();
	    ...
	    jobject javaRegion = env->NewObject(javaClass, methodId,
		  env->NewStringUTF(iter->name.c_str()), type, fileOffset);
   ...
   /*
    ...
    if (!nativeDatabase)
    {
    printf("jniGetNode(): NULL Database object");
    return NULL;
    }
   ...
   jobject
   Java_osm_scout_Database_jniGetObjects(JNIEnv *env, jobject object, int databaseIndex,
	 int objectTypeSetsIndex, double lonMin, double latMin, double lonMax, double latMax,
	 double magnification) {
      printf("jniGetObjects(): GetObjects() starts...");

      Database *nativeDatabase = gDatabaseArray->Get(databaseIndex);

      if (!nativeDatabase) {
	 printf("jniGetObjects(): NULL Database object");
	 return NULL;
      }

      ObjectTypeSets *nativeObjectTypeSets = gObjectTypeSetsArray->Get(objectTypeSetsIndex);

      if (!nativeObjectTypeSets) {
	 printf("jniGetObjects(): NULL ObjectTypeSets object");
	 return NULL;
      }
      ...
      if (!nativeDatabase->GetObjects(nativeObjectTypeSets->nodeTypes,
	    ...
	    nativeMapData->areas)) {
	 printf("jniGetObjects(): GetObjects() Failed!");
	 ...
	 printf("jniGetObjects(): GetObjects() Ok!");
   ...
   jboolean
   Java_osm_scout_Database_jniGetGroundTiles(JNIEnv *env, jobject object, int databaseIndex,
	 int mapDataIndex, double lonMin, double latMin, double lonMax, double latMax,
	 double magnification) {
      printf("jniGetGroundTiles(): GetGroundTiles() starts...");

      Database *nativeDatabase = gDatabaseArray->Get(databaseIndex);

      if (!nativeDatabase) {
	 printf("jniGetGroundTiles(): NULL Database object");
	 return JNI_FALSE;
      }
      ...
      if (!nativeMapData) {
	 printf("jniGetGroundTiles(): NULL MapData object");
	 return JNI_FALSE;
      }
   ...
   jobject
   Java_osm_scout_Database_jniGetTypeConfig(JNIEnv *env, jobject object, int databaseIndex) {
      Database *nativeDatabase = gDatabaseArray->Get(databaseIndex);

      if (!nativeDatabase) {
	 printf("jniGetTypeConfig(): NULL Database object");
	 return NULL;
      }
